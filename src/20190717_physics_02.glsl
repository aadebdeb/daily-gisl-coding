...
float SPHERE_OFFSETS[SPHERE_NUM];
void initConstants() {
    SPHERE_SPECS[0] = vec3(0.8, 0.1, 0.1);
    SPHERE_SPECS[1] = vec3(0.1, 0.8, 0.1);
    SPHERE_SPECS[2] = vec3(0.1, 0.1, 0.8);
    SPHERE_OFFSETS[0] = -5.0;
    SPHERE_OFFSETS[1] = 0.0;
    SPHERE_OFFSETS[2] = 5.0;
}
...
float distSpheres(vec3 p, float r) {
    ...
    for (int i = 0; i < SPHERE_NUM; i++) {
        float angle = 0.3 * PI * sin(SPHERE_SPEED * (time + 0.5 * float(i)));
        vec3 offset = vec3(
...
float distGround(vec3 p) {
    float rd = sdRect(p.xz, vec2(1.5 * GROUND_RADIUS, GROUND_RADIUS));
    float cd = length(p.xy) - GROUND_RADIUS;
    return max(max(rd, p.y + 4.0), -cd);
}
...
void hitGround(vec3 ro, vec3 rd, inout Surface surf) {
    ...
    for (int i = 0; i < 24; i++) {
        ...
        if (d < 0.01) {
            ...
            surf.diffColor = vec3(0.7);
            surf.specColor = vec3(0.0);
            surf.diff = 1.0;
            surf.spec = 0.0;
            break;
        }
...
void main(void) {
    ...
    vec3 ta = vec3(0.0, -0.8 * GROUND_RADIUS, 0.0);
